...
// node_modules/@memorystack/sdk/dist/errors.js
var require_errors = __commonJS({
  "node_modules/@memorystack/sdk/dist/errors.js"(exports2) {
    "use strict";
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.NetworkError = exports2.NotFoundError = exports2.ValidationError = exports2.RateLimitError = exports2.AuthenticationError = exports2.MemoryStackError = void 0;
    var MemoryStackError = class extends Error {
      constructor(message, statusCode, details) {
        super(message);
        this.name = "MemoryStackError";
    ...
    exports2.MemoryStackError = MemoryStackError;
    var AuthenticationError = class extends MemoryStackError {
    ...
    var RateLimitError = class extends MemoryStackError {
      constructor(message = "Rate limit exceeded", limit, remaining, reset) {
        super(message, 429);
    ...
    var ValidationError = class extends MemoryStackError {
    ...
    var NotFoundError = class extends MemoryStackError {
    ...
    var NetworkError = class extends MemoryStackError {
      constructor(message, originalError) {
        super(message, void 0, originalError);
...
// node_modules/@memorystack/sdk/dist/client.js
var require_client = __commonJS({
  "node_modules/@memorystack/sdk/dist/client.js"(exports2) {
    "use strict";
    var __importDefault = exports2 && exports2.__importDefault || function(mod) {
      return mod && mod.__esModule ? mod : { "default": mod };
    };
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.MemoryStackClient = void 0;
    var axios_1 = __importDefault(require_axios());
    var errors_1 = require_errors();
    var MemoryStackClient = class {
      constructor(config) {
        this.sdkVersion = "1.0.3";
        if (!config.apiKey) {
          throw new errors_1.ValidationError("API key is required");
        }
        const baseUrlInput = config.baseUrl || "https://www.memorystack.app";
        ...
        this.logger = config.logger ?? {
          ...
          info: (msg, data) => console.log(`[MemoryOS] ${msg}`, data || ""),
          warn: (msg, data) => console.warn(`[MemoryOS] ${msg}`, data || ""),
          error: (msg, data) => console.error(`[MemoryOS] ${msg}`, data || "")
        };
        this.retryConfig = {
          maxRetries: config.retryConfig?.maxRetries ?? 3,
          retryDelay: config.retryConfig?.retryDelay ?? 1e3,
          maxRetryDelay: config.retryConfig?.maxRetryDelay ?? 1e4,
        ...
        this.client = axios_1.default.create({
          ...
          headers: {
            ...
            "User-Agent": `memorystack-nodejs-sdk/${this.sdkVersion}`
          },
          timeout: this.timeout
        });
        if (!this.agentName) {
          const detected = this.detectAgentFramework();
          if (detected) {
            this.agentName = detected.name;
            this.agentType = detected.type;
          }
        }
        if (this.agentName) {
          this.initializeAgent(config.projectName || "default").catch((err) => {
            console.warn("Failed to initialize agent:", err.message);
          });
        }
        this.client.interceptors.request.use((config2) => {
          ...
          if (this.enableLogging) {
            this.logger.debug?.("Request", {
              ...
              url: config2.url,
              requestId,
              headers: { ...config2.headers, Authorization: "[REDACTED]" }
            });
          }
          return config2;
        }, (error) => {
        ...
        this.client.interceptors.response.use((response) => {
          ...
          const isRetryableStatus = error.response && this.retryConfig.retryableStatusCodes.includes(error.response.status);
          const shouldRetry = (isNetworkError || isRetryableStatus) && retryCount < this.retryConfig.maxRetries;
          if (shouldRetry) {
            const delay = Math.min(this.retryConfig.retryDelay * Math.pow(2, retryCount), this.retryConfig.maxRetryDelay);
            if (this.enableLogging) {
              this.logger.warn?.(`Retrying request (attempt ${retryCount + 1}/${this.retryConfig.maxRetries}) after ${delay}ms`, {
          ...
          if (error.response) {
            ...
            switch (status) {
              case 401:
                throw new errors_1.AuthenticationError(message, details);
              case 429:
                throw new errors_1.RateLimitError(message, limit ? parseInt(limit) : void 0, remaining ? parseInt(remaining) : void 0, reset ? parseInt(reset) : void 0);
              case 400:
                throw new errors_1.ValidationError(message, details);
              case 404:
                throw new errors_1.NotFoundError(message);
              default:
                throw new errors_1.MemoryStackError(message, status, details);
            }
          }
          if (error.request) {
            throw new errors_1.NetworkError("Network request failed", error);
          }
          throw new errors_1.MemoryStackError(error.message);
        });
      }
      /**
       ...
       * @private
       */
      detectAgentFramework() {
        if (typeof global !== "undefined" && global.__crewai_agent__) {
          const agent = global.__crewai_agent__;
          return {
            name: agent.role || "CrewAI Agent",
            type: agent.role || "crewai"
          };
        }
        if (typeof global !== "undefined" && global.__langgraph_agent__) {
          const agent = global.__langgraph_agent__;
          return {
            name: agent.name || "LangGraph Agent",
            type: "langgraph"
          };
        }
        if (typeof global !== "undefined" && global.__autogpt_agent__) {
          const agent = global.__autogpt_agent__;
          return {
            name: agent.name || "AutoGPT Agent",
            type: "autonomous"
          };
        }
        return null;
       ...
       * @private
       */
      async initializeAgent(projectName) {
        try {
          console.log(`Agent "${this.agentName}" will be initialized on first memory creation`);
        } catch (error) {
          console.warn("Agent initialization note:", error);
        }
      }
      /**
       ...
       * await client.add("User prefers dark mode", { userId: "user_123" });
       *
       * // Conversation format
       * await client.add([
      ...
      async add(content, options) {
        ...
        if (typeof content === "string") {
          ...
          throw new errors_1.ValidationError("Content must be a string or array of messages");
        }
        if (messages.length === 0) {
          throw new errors_1.ValidationError("At least one message is required");
        }
        if (messages.length > 100) {
          throw new errors_1.ValidationError("Maximum 100 messages per request");
        }
        for (const msg of messages) {
          if (!msg.role || !msg.content) {
            throw new errors_1.ValidationError(`Message missing role or content: ${JSON.stringify(msg)}`);
          }
        }
        const payload = {
          messages,
        ...
        const endpoint = this.agentName ? "/memories/agents" : "/memories";
      ...
      async createMemory(request) {
        return this.add(request.messages, {
      ...
      async listMemories(request = {}) {
        if (request.limit !== void 0) {
          if (!Number.isInteger(request.limit) || request.limit < 1 || request.limit > 100) {
            throw new errors_1.ValidationError(`Invalid limit: ${request.limit}. Must be an integer between 1 and 100`);
          }
        }
        if (request.order && !["asc", "desc"].includes(request.order)) {
          throw new errors_1.ValidationError(`Invalid order: "${request.order}". Must be 'asc' or 'desc'`);
        }
        if (request.min_confidence !== void 0) {
          if (typeof request.min_confidence !== "number" || request.min_confidence < 0 || request.min_confidence > 1) {
            throw new errors_1.ValidationError(`Invalid min_confidence: ${request.min_confidence}. Must be a number between 0 and 1`);
          }
        }
        const params = new URLSearchParams();
        if (request.user_id !== void 0) {
        ...
        if (request.memory_type) {
          params.append("memory_type", request.memory_type);
        }
        if (request.min_confidence !== void 0) {
          params.append("min_confidence", request.min_confidence.toString());
        }
        if (request.include_embedding !== void 0) {
          params.append("include_embedding", request.include_embedding.toString());
        }
       ...
       * @returns Usage stats including API calls, storage, and limits
       */
      async getStats() {
      ...
      /**
       * Get knowledge graph data (requires Clerk authentication, not API key)
       * Note: This endpoint is typically used from the dashboard UI
       ...
       * @deprecated Use search() for finding specific memories
       */
      async list(options) {
      ...
      // Legacy helper methods - kept for backward compatibility but deprecated
      /** @deprecated Use add() instead */
      async addConversation(userMessage, assistantMessage, userId, metadata) {
        return this.add([{ role: "user", content: userMessage }, { role: "assistant", content: assistantMessage }], { userId, metadata });
      }
      /** @deprecated Use add() instead */
      async addMessage(message, userId, metadata) {
        return this.add(message, { userId, metadata });
      }
      /** @deprecated Use list() instead */
      async getUserMemories(userId, limit = 20) {
        return this.listMemories({ user_id: userId, limit });
      }
      /** @deprecated Use list() instead */
      async getPersonalMemories(limit = 20) {
        return this.listMemories({ user_id: "self", limit });
      }
      /** @deprecated Use list() instead */
      async getAllMemories(limit = 20) {
        return this.listMemories({ user_id: "all", limit });
      }
      /**
       * Get a single memory by ID
       * @param memoryId - Memory ID to retrieve
       * @returns Memory object
       * @throws {NotFoundError} if memory not found
       * @throws {MemoryStackError} if endpoint doesn't exist (404) - GET endpoint may need to be implemented
       */
      async getMemory(memoryId) {
        try {
          ...
          if (error.statusCode === 404 && error.message?.includes("404")) {
            throw new errors_1.NotFoundError(`Memory not found or GET endpoint not implemented. Memory ID: ${memoryId}`);
          }
          throw error;
        }
      }
      ...
      async updateMemory(memoryId, updates) {
        if (!memoryId || memoryId.trim().length === 0) {
          throw new errors_1.ValidationError("Memory ID is required");
        }
        if (!updates.content && updates.memory_type === void 0 && updates.confidence === void 0 && updates.metadata === void 0) {
          throw new errors_1.ValidationError("At least one field must be provided for update");
        }
        if (updates.content !== void 0 && updates.content.trim().length === 0) {
          throw new errors_1.ValidationError("Content cannot be empty");
        }
        if (updates.confidence !== void 0) {
          if (typeof updates.confidence !== "number" || updates.confidence < 0 || updates.confidence > 1) {
            throw new errors_1.ValidationError(`Invalid confidence: ${updates.confidence}. Must be a number between 0 and 1`);
          }
        }
       ...
       * @param hard - Permanently delete (true) or soft delete (false)
       */
      async deleteMemory(memoryId, hard = false) {
        if (!memoryId || memoryId.trim().length === 0) {
          throw new errors_1.ValidationError("Memory ID is required");
        }
        const response = await this.client.delete(`/memories/${memoryId}?hard=${hard}`);
        return response.data;
      }
      /**
       ...
       * @param hard - Permanently delete (true) or soft delete (false)
       */
      async deleteMemories(memoryIds, hard = false) {
        const response = await this.client.delete("/memories/batch", {
          data: {
       ...
       *
       * // With user ID (for B2B apps)
       * const results = await client.search("user preferences", { userId: "user_123" });
       *
       * // With limit
      ...
      async search(query, options) {
        ...
        const response = await this.client.get(`/memories/search?${params.toString()}`);
        return response.data;
      }
      /**
       ...
       */
      async listAgentMemories(options) {
        if (options?.limit !== void 0) {
          if (!Number.isInteger(options.limit) || options.limit < 1 || options.limit > 100) {
            throw new errors_1.ValidationError(`Invalid limit: ${options.limit}. Must be an integer between 1 and 100`);
          }
        }
        if (!this.agentId) {
          throw new Error("Agent not initialized. Create a memory first or provide agentName in constructor.");
        }
        const params = new URLSearchParams();
        params.append("agent_id", this.agentId);
        if (options?.limit)
          params.append("limit", options.limit.toString());
        if (options?.cursor)
          params.append("cursor", options.cursor);
        if (options?.includeTeam !== void 0)
          params.append("include_team", options.includeTeam.toString());
        if (options?.includeProject !== void 0)
          params.append("include_project", options.includeProject.toString());
        const response = await this.client.get(`/memories/agents?${params.toString()}`);
        return response.data;
      }
      /**
      ...
      async detectContradictions(options) {
        if (!options.content || options.content.trim().length === 0) {
          throw new errors_1.ValidationError("Content is required for contradiction detection");
        }
        if (options.confidence !== void 0) {
          if (typeof options.confidence !== "number" || options.confidence < 0 || options.confidence > 1) {
            throw new errors_1.ValidationError(`Invalid confidence: ${options.confidence}. Must be a number between 0 and 1`);
          }
        }
        const response = await this.client.post("/memories/detect-contradictions", {
      ...
      /**
       * Update belief when new evidence contradicts existing memory
       * @param options - Update options
       */
      async updateBelief(options) {
        if (!options.content || options.content.trim().length === 0) {
          throw new errors_1.ValidationError("Content is required for belief update");
        }
        if (options.confidence !== void 0) {
          if (typeof options.confidence !== "number" || options.confidence < 0 || options.confidence > 1) {
            throw new errors_1.ValidationError(`Invalid confidence: ${options.confidence}. Must be a number between 0 and 1`);
        ...
        if (options.strategy && !["auto", "supersede", "merge", "adjust", "manual"].includes(options.strategy)) {
          throw new errors_1.ValidationError(`Invalid strategy: "${options.strategy}". Must be 'auto', 'supersede', 'merge', 'adjust', or 'manual'`);
        }
        const response = await this.client.post("/memories/update-belief", {
          content: options.content,
          user_id: options.userId,
          confidence: options.confidence ?? 0.8,
          strategy: options.strategy ?? "auto",
          contradicting_memory_id: options.contradictingMemoryId,
          metadata: options.metadata
        });
        return response.data;
      }
      /**
       * Get complete provenance and lineage of a memory
       * @param memoryId - Memory ID to get lineage for
       * @param format - Response format ('full', 'timeline', or 'chain')
       */
      async getMemoryLineage(memoryId, format = "full") {
        if (!memoryId || memoryId.trim().length === 0) {
          throw new errors_1.ValidationError("Memory ID is required");
        }
        if (!["full", "timeline", "chain"].includes(format)) {
          throw new errors_1.ValidationError(`Invalid format: "${format}". Must be 'full', 'timeline', or 'chain'`);
        }
        const response = await this.client.get(`/memories/${memoryId}/lineage?format=${format}`);
        return response.data;
      }
      /**
       * Create multiple memories at once (batch operation)
       * @param memories - Array of memory objects
       */
      async createMemoriesBatch(memories) {
        if (!memories || memories.length === 0) {
          throw new errors_1.ValidationError("At least one memory is required for batch creation");
        }
        if (memories.length > 1e3) {
          throw new errors_1.ValidationError("Maximum 1000 memories per batch request");
        }
        for (let i = 0; i < memories.length; i++) {
          const memory = memories[i];
          if (!memory.content || memory.content.trim().length === 0) {
            throw new errors_1.ValidationError(`Memory at index ${i} is missing content`);
          }
          if (memory.confidence !== void 0) {
            if (typeof memory.confidence !== "number" || memory.confidence < 0 || memory.confidence > 1) {
              throw new errors_1.ValidationError(`Invalid confidence at index ${i}: ${memory.confidence}. Must be between 0 and 1`);
            }
          }
        }
        const response = await this.client.post("/memories/batch/create", {
          memories,
      ...
      async exportMemories(options) {
        ...
        if (options?.minConfidence !== void 0)
          payload.min_confidence = options.minConfidence;
        if (options?.includeEmbeddings)
          payload.include_embeddings = options.includeEmbeddings;
        if (options?.dateFrom)
          payload.date_from = options.dateFrom;
        if (options?.dateTo)
          payload.date_to = options.dateTo;
        const response = await this.client.post("/memories/batch/export", payload);
        return response.data;
      }
      /**
      ...
      async cancelBatchJob(jobId) {
        const response = await this.client.post(`/batch-jobs/${jobId}/cancel`);
        return response.data;
      }
      /**
       * Retry a failed batch job
       * @param jobId - Batch job ID to retry
       */
      async retryBatchJob(jobId) {
        const response = await this.client.post(`/batch-jobs/${jobId}/retry`);
        return response.data;
      }
      /**
      ...
      /**
       * Generate or regenerate embeddings for memories
       * @param options - Embedding generation options
       */
      async generateEmbeddingsBatch(options) {
        if (options?.memoryIds) {
          if (options.memoryIds.length === 0) {
            throw new errors_1.ValidationError("memoryIds array cannot be empty");
          }
          if (options.memoryIds.length > 1e3) {
            throw new errors_1.ValidationError("Maximum 1000 memory IDs per request");
          }
        }
        const payload = {
          regenerate_all: options?.regenerateAll || false
        };
        if (options?.memoryIds)
          payload.memory_ids = options.memoryIds;
        if (options?.userId)
          payload.user_id = options.userId;
        if (options?.memoryType)
          payload.memory_type = options.memoryType;
        const response = await this.client.post("/memories/batch/embeddings", payload);
        return response.data;
      }
      /**
      ...
      async getEmbeddingStatistics() {
        const response = await this.client.get("/memories/batch/embeddings");
        return response.data;
      }
      /**
       ...
       */
      async reflectOnMemories(options) {
        if (options?.timeWindowDays !== void 0) {
          if (!Number.isInteger(options.timeWindowDays) || options.timeWindowDays < 1 || options.timeWindowDays > 90) {
            throw new errors_1.ValidationError(`Invalid timeWindowDays: ${options.timeWindowDays}. Must be an integer between 1 and 90`);
          }
        }
        if (options?.analysisDepth && !["shallow", "deep"].includes(options.analysisDepth)) {
          throw new errors_1.ValidationError(`Invalid analysisDepth: "${options.analysisDepth}". Must be 'shallow' or 'deep'`);
        }
        if (options?.minPatternStrength !== void 0) {
          if (typeof options.minPatternStrength !== "number" || options.minPatternStrength < 0 || options.minPatternStrength > 1) {
            throw new errors_1.ValidationError(`Invalid minPatternStrength: ${options.minPatternStrength}. Must be a number between 0 and 1`);
          }
        }
        const payload = {
          timeWindowDays: options?.timeWindowDays || 7,
          analysisDepth: options?.analysisDepth || "shallow",
          dryRun: options?.dryRun || false,
          minPatternStrength: options?.minPatternStrength || 0.7,
          useBatch: options?.useBatch || false
        };
        const response = await this.client.post("/memories/reflect", payload);
        return response.data;
      }
      /**
      ...
      async consolidateMemories(options) {
        if (options?.similarityThreshold !== void 0) {
          if (typeof options.similarityThreshold !== "number" || options.similarityThreshold < 0 || options.similarityThreshold > 1) {
            throw new errors_1.ValidationError(`Invalid similarityThreshold: ${options.similarityThreshold}. Must be a number between 0 and 1`);
          }
        }
        const payload = {
          similarity_threshold: options?.similarityThreshold || 0.9,
          check_redundancy: options?.checkRedundancy !== false,
          dry_run: options?.dryRun || false,
          use_batch: options?.useBatch || false
        };
        const response = await this.client.post("/memories/consolidate", payload);
        return response.data;
      }
      // Auto-maintenance configuration methods
      /**
       * Configure automatic memory consolidation
       */
      async configureAutoConsolidation(options) {
        const payload = {
          consolidation_enabled: options.enabled,
          consolidation_frequency: options.frequency || "weekly",
          consolidation_similarity_threshold: options.similarityThreshold || 0.95,
          consolidation_notify_on_merge: options.notifyOnMerge || false
        };
        const response = await this.client.post("/auto-maintenance/config", payload);
        return response.data;
      }
      /**
       * Configure automatic memory reflection
       */
      async configureAutoReflection(options) {
        const payload = {
          reflection_enabled: options.enabled,
          reflection_frequency: options.frequency || "weekly",
          reflection_time_window_days: options.timeWindowDays || 7,
          reflection_analysis_depth: options.analysisDepth || "shallow",
          reflection_min_pattern_strength: options.minPatternStrength || 0.7
        };
        const response = await this.client.post("/auto-maintenance/config", payload);
        return response.data;
      }
       ...
       */
      async configureAutoEmbeddings(options) {
        const payload = {
          embedding_generation_enabled: options.enabled,
          embedding_check_frequency: options.checkFrequency || "daily"
        };
        const response = await this.client.post("/auto-maintenance/config", payload);
        return response.data;
      }
       ...
       */
      async getAutoMaintenanceConfig() {
        const response = await this.client.get("/auto-maintenance/config");
        return response.data;
      }
      /**
       * Update auto-maintenance configuration
       */
      async updateAutoMaintenanceConfig(config) {
        const response = await this.client.post("/auto-maintenance/config", config);
        return response.data;
      }
      /**
       * Disable all auto-maintenance features
       */
      async disableAutoMaintenance() {
        const payload = {
      ...
      /**
       * Reset auto-maintenance configuration to defaults
       */
      async resetAutoMaintenanceConfig() {
        const response = await this.client.delete("/auto-maintenance/config");
        return response.data;
      }
    };
    exports2.MemoryStackClient = MemoryStackClient;
  }
});

// node_modules/@memorystack/sdk/dist/types.js
var require_types = __commonJS({
  "node_modules/@memorystack/sdk/dist/types.js"(exports2) {
    "use strict";
    Object.defineProperty(exports2, "__esModule", { value: true });
  }
});

// node_modules/@memorystack/sdk/dist/index.js
var require_dist = __commonJS({
  "node_modules/@memorystack/sdk/dist/index.js"(exports2) {
    "use strict";
    ...
    Object.defineProperty(exports2, "MemoryStackClient", { enumerable: true, get: function() {
      return client_1.MemoryStackClient;
    } });
...
// src/lib/memorystack-client.js
var require_memorystack_client = __commonJS({
  "src/lib/memorystack-client.js"(exports2, module2) {
    var { MemoryStackClient } = require_dist();
    var crypto = require("crypto");
...
Preserve the content as-is with minimal processing.`;
    var MemoryStackClientWrapper2 = class {
      ...
      getProjectScopeId() {
        try {
          ...
          const cleaned = remote.replace(/^.*[:/]/, "").replace(/\.git$/, "").replace(/[^a-zA-Z0-9_-]/g, "_").toLowerCase();
      ...
      async addMemory(content, opts = {}) {
        ...
        if (extractionContext) {
          ...
          metadata.extraction_context = scope === "project" ? PROJECT_ENTITY_CONTEXT : PERSONAL_ENTITY_CONTEXT;
        }
      ...
      async getProfile() {
        ...
        const personal = personalResult.status === "fulfilled" ? personalResult.value.results : [];
        const project = projectResult.status === "fulfilled" ? projectResult.value.results : [];
        let recent = [];
        try {
    ...
    module2.exports = {
      MemoryStackClientWrapper: MemoryStackClientWrapper2,
...
var require_compress = __commonJS({
  "src/lib/compress.js"(exports2, module2) {
    ...
    module2.exports = { compressObservation, getObservationMetadata };
  }
});
...
var require_transcript_formatter = __commonJS({
  "src/lib/transcript-formatter.js"(exports2, module2) {
    ...
    var STATE_DIR = path.join(os.homedir(), ".memorystack-claude");
    var STATE_FILE = path.join(STATE_DIR, "capture-state.json");
    function getCaptureState(projectKey) {
      try {
        if (fs2.existsSync(STATE_FILE)) {
          const data = JSON.parse(fs2.readFileSync(STATE_FILE, "utf8"));
    ...
    function saveCaptureState(projectKey, state) {
      try {
        ...
        try {
          if (fs2.existsSync(STATE_FILE)) {
            data = JSON.parse(fs2.readFileSync(STATE_FILE, "utf8"));
          }
        } catch {
        }
        data[projectKey] = { ...state, updatedAt: (/* @__PURE__ */ new Date()).toISOString() };
        fs2.writeFileSync(STATE_FILE, JSON.stringify(data, null, 2));
      } catch {
      }
    }
    ...
    function getTurnsAroundSignals(turns, signalIndices, turnsBefore = 1) {
      ...
      return Array.from(includeSet).sort((a, b) => a - b).map((i) => turns[i]);
    }
    function formatTurn(turn, options = {}) {
      ...
      if (includeTools && turn.tools.length > 0) {
        const relevantTools = captureTools.length > 0 ? turn.tools.filter((t) => captureTools.some((ct) => t.name.toLowerCase().includes(ct.toLowerCase()))) : turn.tools;
        if (relevantTools.length > 0) {
          const compressed = relevantTools.map(
            (t) => compressObservation(t.name, t.input, t.output)
          );
    ...
    function formatNewEntries(transcriptContent, projectKey, options = {}) {
      ...
      const formatted = turns.map((t) => formatTurn(t, options)).filter((f) => f.length > 20).join("\n---\n");
    ...
    function formatSignalEntries(transcriptContent, projectKey, options = {}) {
      ...
      const formatted = relevantTurns.map((t) => formatTurn(t, { includeTools: true, captureTools })).filter((f) => f.length > 20).join("\n---\n");
      if (!formatted || formatted.length < 50) return null;
      const maxIndex = Math.max(...entries.map((e) => e.index));
      setLastCapturedIndex(projectKey, maxIndex + 1);
      return formatted.slice(0, 4e3);
    }
    function formatTranscriptSmart2(transcriptContent, projectKey, options = {}) {
    ...
    module2.exports = {
      ...
      formatNewEntries,
      formatSignalEntries,
      formatTranscriptSmart: formatTranscriptSmart2,
...
// src/summary-hook.js
var fs = require("fs");
var { MemoryStackClientWrapper } = require_memorystack_client();
var { loadSettings, getApiKey, debugLog, getProjectName } = require_settings();
var { readStdin, writeOutput } = require_stdin();
var { formatTranscriptSmart } = require_transcript_formatter();
async function main() {
