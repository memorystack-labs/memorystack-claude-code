...
var require_auth = __commonJS({
  "src/lib/auth.js"(exports2, module2) {
    var http = require("http");
    var { exec } = require("child_process");
    var { saveApiKey } = require_settings();
    var AUTH_URL = "https://memorystack.app/dashboard/api-keys";
    var AUTH_TIMEOUT = 12e4;
    async function startAuthFlow2() {
      return new Promise((resolve, reject) => {
    ...
    module2.exports = { startAuthFlow: startAuthFlow2 };
  }
});
...
Session will continue without memory context.
    ...
    function formatAuthRequired2(isTimeout = false) {
      return `<memorystack-status>
...
https://memorystack.app/dashboard/api-keys
    ...
    module2.exports = {
      formatProfileContext: formatProfileContext2,
      formatContext,
      formatRelativeTime,
...
var { startAuthFlow } = require_auth();
var { formatProfileContext, formatEmptyContext, formatErrorContext, formatAuthRequired } = require_format_context();
async function main() {
  ...
  try {
    ...
    try {
      ...
      const totalMemories = (profile.personal?.length || 0) + (profile.project?.length || 0) + (profile.recent?.length || 0);
      ...
      additionalContext = totalMemories > 0 ? formatProfileContext(profile) : formatEmptyContext();
    } catch (err) {
      ...
      try {
        ...
        const { formatContext } = require_format_context();
        additionalContext = results.results?.length > 0 ? formatContext(results.results, false) : formatEmptyContext();
      } catch {
